...
function reverseString(str) {
  ...
  return tail+reverseString(head);
}
...
function fileFinder(directories, targetFile) {
  if(!directories) { 
    return false; 
  } else if(directories.hasOwnProperty(targetFile)) {
    return true;
  } else {
    const files = Object.keys(directories);
    const dirs = files.filter( path => path[0] === "/" );

    for(let i = 0; i < dirs.length; i++ ) {
      const dirPath = dirs[i];
      const subDir = directories[dirPath];
      if(fileFinder(subDir, targetFile))
        return true;
    }
    return false;
  }
...
function pathFinder(directories, targetFile) {
  ...
  for(let i = 0; i < dirs.length; i++ ) {
    const dirPath = dirs[i];
    const subDir = directories[dirPath];
    const filePath = pathFinder(subDir, targetFile);
    if(filePath) return dirPath+filePath;
  }
  return null;
}
